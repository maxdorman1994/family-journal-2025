...
async function setupR2Configuration() {
  ...
  console.log('This script will help you configure photo storage for cross-device sync.\n');

  try {
    // Collect R2 configuration
    console.log('📋 Please provide your Cloudflare R2 details:\n');
    
    const accountId = await askQuestion('1. Account ID (from Cloudflare dashboard): ');
    const accessKeyId = await askQuestion('2. R2 Access Key ID: ');
    const secretAccessKey = await askQuestion('3. R2 Secret Access Key: ');
    const bucketName = await askQuestion('4. Bucket Name (default: wee-adventure-photos): ') || 'wee-adventure-photos';
    
    console.log('\n🌐 Optional: Custom domain configuration');
    const customDomain = await askQuestion('5. Custom domain for photos (optional, press Enter to skip): ');
