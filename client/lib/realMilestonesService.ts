 ...
 */
export const MILESTONE_TEMPLATES: Omit<RealMilestone, 'id' | 'current_value' | 'completed' | 'progress_percentage' | 'completed_date'>[] = [
  // Explorer Milestones
  {
...
export async function calculateRealMilestones(): Promise<RealMilestone[]> {
  try {
    ...
    console.log(`📖 Processing ${journalEntries.length} journal entries for milestones...`);

    // Calculate stats from real data
    const stats = {
      total_adventures: journalEntries.length,
      total_photos: journalEntries.reduce((sum, entry) => sum + (entry.photos?.length || 0), 0),
      total_distance: journalEntries.reduce((sum, entry) => sum + (entry.miles_traveled || 0), 0),
      unique_locations: new Set(journalEntries.map(entry => entry.location?.toLowerCase().trim())).size,
      unique_weather: new Set(journalEntries.map(entry => entry.weather?.toLowerCase().trim())).size,
      dog_friendly_count: journalEntries.filter(entry => entry.dog_friendly === true).length,
      paid_activities_count: journalEntries.filter(entry => entry.paid_activity === true).length,
    };
    ...
    const milestones: RealMilestone[] = MILESTONE_TEMPLATES.map(template => {
      ...
      switch (template.category) {
        case 'explorer':
          if (template.title.includes('adventure') || template.title.includes('Adventure')) {
            currentValue = stats.total_adventures;
          }
          break;
          
        case 'photographer':
          if (template.title.includes('photo') || template.title.includes('Photo')) {
            currentValue = stats.total_photos;
          }
          break;
          
        case 'distance':
          if (template.title.includes('mile') || template.title.includes('Travel')) {
      ...
      const progress_percentage = Math.min((currentValue / template.target_value) * 100, 100);
    ...
    console.log(`✅ Generated ${milestones.length} milestones, ${milestones.filter(m => m.completed).length} completed`);
...
export async function getRealMilestoneStats(): Promise<MilestoneStats> {
  try {
    ...
    const stats: MilestoneStats = {
      ...
      total_distance: journalEntries?.reduce((sum, entry) => sum + (entry.miles_traveled || 0), 0) || 0,
      total_locations: new Set(journalEntries?.map(entry => entry.location?.toLowerCase().trim())).size || 0,
      total_photos: journalEntries?.reduce((sum, entry) => sum + (entry.photos?.length || 0), 0) || 0,
      unique_weather_conditions: new Set(journalEntries?.map(entry => entry.weather?.toLowerCase().trim())).size || 0,
      dog_friendly_adventures: journalEntries?.filter(entry => entry.dog_friendly === true).length || 0,
      paid_activities: journalEntries?.filter(entry => entry.paid_activity === true).length || 0,
 ...
 */
export function subscribeToRealMilestones(callback: (milestones: RealMilestone[]) => void): () => void {
 ...
 */
export function filterMilestonesByCategory(milestones: RealMilestone[], category: string): RealMilestone[] {
 ...
 */
export function getMilestonesByStatus(milestones: RealMilestone[], completed: boolean): RealMilestone[] {
...
function generateMilestoneId(title: string): string {
  return `milestone_${title.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '')}`;
}

/**
 * Get next milestone suggestions
 */
export function getNextMilestones(milestones: RealMilestone[]): RealMilestone[] {
  return milestones
 ...
 */
export function getRecentlyCompleted(milestones: RealMilestone[]): RealMilestone[] {
  return milestones
