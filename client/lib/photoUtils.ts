...
export async function processPhoto(file: File): Promise<ProcessedPhoto> {
  ...
  try {
    ...
    console.log(
      `Photo processed: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)}MB) -> ${compressedFile.name} (${(compressedFile.size / 1024 / 1024).toFixed(2)}MB)`,
    );
...
/**
 * Upload photo to Cloudflare R2 via our API
 */
