-- Hidden Gems Database Schema (Safe Version) - Handles existing objects gracefully
...
-- Create view for hidden gem statistics (replace if exists)
...
-- Drop existing triggers and function safely, then recreate
DROP TRIGGER IF EXISTS trigger_update_hidden_gems_updated_at ON hidden_gems;
DROP TRIGGER IF EXISTS trigger_update_hidden_gem_visits_updated_at ON hidden_gem_visits;
DROP FUNCTION IF EXISTS update_hidden_gem_updated_at();
...
-- Success message
SELECT 'Hidden Gems database schema created/updated successfully!' as status;
